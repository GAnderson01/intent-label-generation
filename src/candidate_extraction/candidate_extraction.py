...
class Extractor:
    def __init__(self, dataset, extraction_method):
        """
        ...
        if extraction_method in ['action-object', 'action-object-ext']: 
            self.nlp = spacy.load("en_core_web_sm")

        candidates_dir = "../data/interim/candidates/"
        extraction_df_filename = "_".join([dataset, extraction_method, "extraction_df.csv"])
    ...
    def t0pp_prompting_get_candidates(self, df):
        ...
        Returns:
            pd.DataFrame: original dataframe with added candidate_intent column 
        """   
        t0pp_load_start_time = time.time()
        import utils.t0pp as T0ppModel
        t0pp_load_end_time = time.time()
        print("T0pp load time: %.2f." % (t0pp_load_end_time-t0pp_load_start_time))
