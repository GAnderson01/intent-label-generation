...
class IntentLabelGenerator:
    def __init__(self, dataset, extraction_method, generation_method):
        """
        ...
        Args:
            ...
            generation_method (str): chosen intent label selection method. Options: most-frequent, t0pp-prompting
        """        
    ...
    def generate(self, df, cluster_labels, verbose=False):
        ...
        Args:
            ...
            cluster_labels (np.array/list): numpy array or list of cluster labels (obtained from the DeepAligned cluster model)
        ...
        if self.generation_method == "most-frequent":
            df = self.get_n_most_frequest(df)
    ...
    def get_n_most_frequest(self, df):
        """Choose intent label as the most frequent candidate in each cluster
        ...
        for cluster_id in cluster_id_set:
            ...
            most_frequent = pair_counter.most_common(1)
    ...
    def t0pp_prompting_get_intents(self, df, n, verbose=False):
        ...
        all_cluster_utterances = order_kmeans_clusters(df, cluster_id_set, self.dataset)
        
        cluster_id_to_generated = {}
        for c in tqdm(cluster_id_set):
            ...
            candidates = [str(cand[0]) for cand in candidates_counter.most_common(n)]
...
def order_kmeans_clusters(df, cluster_ids, dataset):
    ...
    embeddings_path = os.path.join(embeddings_dir, f"{dataset}_embeddings.pkl")
    embeddings = load(embeddings_path)

    model_dir = '../models/'
    cluster_model_path = os.path.join(model_dir, f"{dataset}_deep_aligned_cluster_model.pkl")
