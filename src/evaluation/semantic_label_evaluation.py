...
class SemanticLabelEvaluator:
    ...
    def evaluate(self, df, embedding_model):
        ...
        for c in cluster_ids:
            cluster_id_to_most_common_gt[c] = df[df['cluster_id'] == c]['label'].value_counts().index[0]
        ...
        print(f"Generated intents: {generated_intents}")
        # Check if ground truth labels are in pascal_case
        pascal_case = True
        if self.dataset in ['atis', 'banking', 'bitext', 'clinc', 'personal_assistant', 'stack_overflow', 'twacs']:
            pascal_case = False
        ...
        all_intents = df['label'].unique().tolist()
        avg_sim_per_label = {}
